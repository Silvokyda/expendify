        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                ...
                <select name="period" id="period"
                        class="mt-1 w-full rounded-md border-gray-300 dark:bg-gray-800">
                    <option value="monthly" @selected(old('period','monthly')==='monthly')>Monthly</option>
                    <option value="weekly"  @selected(old('period')==='weekly')>Weekly</option>
                    <option value="custom"  @selected(old('period')==='custom')>Custom</option>
                </select>
            ...
            <div class="md:col-span-2">
                ...
                <input name="total_amount" value="{{ old('total_amount') }}" type="number" step="0.01" min="0" required
                       class="mt-1 w-full rounded-md border-gray-300 dark:bg-gray-800" />
                @error('total_amount') <p class="text-sm text-red-600 mt-1">{{ $message }}</p> @enderror
            </div>
        </div>

        <div id="custom-dates" class="grid grid-cols-1 md:grid-cols-2 gap-4 {{ old('period')==='custom' ? '' : 'hidden' }}">
            <div>
