...
<head>
    ...
    <title>Expendify — Forgot Password</title>
...
<div class="grid lg:grid-cols-2 min-h-screen">
    ...
    <main id="main" class="relative flex items-center justify-center px-4 sm:px-6 lg:px-10 py-10">
        <div class="w-full max-w-md">
            ...
            <h1 class="mt-8 text-3xl font-semibold tracking-tight">Forgot your password?</h1>
            ...
            {{-- Status (success) --}}
            @if (session('status'))
                <div class="mt-6 rounded-lg bg-[#ecfdf5] dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-200 ring-1 ring-emerald-300/60 dark:ring-emerald-800 px-4 py-3 text-sm">
                    {{ session('status') }}
                </div>
            @endif
            ...
            <div class="mt-6 rounded-2xl bg-white/70 dark:bg-white/10 backdrop-blur-md ring-1 ring-black/5 dark:ring-white/10 shadow-sm p-5 sm:p-6">
                <form method="POST" action="{{ route('password.email') }}" class="space-y-4">
                    ...
                    <!-- Secondary action -->
                    <a href="{{ route('login') }}"
                       class="w-full inline-flex items-center justify-center rounded-lg bg-black/5 dark:bg-white/10 ring-1 ring-black/10 dark:ring-white/20 hover:bg-black/10 dark:hover:bg-white/15 font-medium px-4 py-2.5">
                        Back to Login
                    </a>
                </form>
            </div>
    ...
    <aside class="relative hidden lg:flex items-center justify-center">
        ...
        <div class="mx-auto max-w-xl">
            <div class="rounded-2xl bg-white/10 dark:bg-white/5 backdrop-blur-xl border border-white/20 shadow-xl p-6">
                <h2 class="text-2xl font-semibold leading-tight">Reset access securely</h2>
                <ul class="mt-4 space-y-2 text-sm text-white/90">
                    <li class="flex items-start gap-2"><span class="mt-1 h-1.5 w-1.5 rounded-full bg-white/70"></span> One-click email reset link</li>
                    <li class="flex items-start gap-2"><span class="mt-1 h-1.5 w-1.5 rounded-full bg-white/70"></span> Protects your account & data</li>
                    <li class="flex items-start gap-2"><span class="mt-1 h-1.5 w-1.5 rounded-full bg-white/70"></span> Fast and reliable delivery</li>
                </ul>
                <p class="mt-3 text-[11px] text-white/60">We never store your PIN or card data.</p>
            </div>
        </div>
    </aside>
</div>
